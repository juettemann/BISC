...
use constant BLASTOUTINTACTTAED  => '/home/tmp/BISC/blast/blast-2.2.19/taed.biscMiHet.blast.out';
...
my $f = '/home/tmp/BISC/taed/biscMi.ppi';
my $FGinteractions;
open(FH,'<',$f) or die "Can not open/access '".$f."'\n$!";
   while(my $line = <FH>){
      chomp($line);
      my @line = split("\t",$line);
      $FGinteractions->{$line[0]}->{$line[1]}++;
      $FGinteractions->{$line[1]}->{$line[0]}++;
   }
   close(FH);


   my $HRintactBlastOutHet   = $pdbaa->parseTaedVsUniprotLike(BLASTOUTINTACTTAED) ; #print Dumper($HRintactBlastOutHet);die; 
   my $HRBlastresults = $HRintactBlastOutHet;
   my $HR;
LOOP:
...
# go top->bottom through blast results for each interaction partner
            foreach my $rank1 (    sort {$HRBlastresults->{$id1}->{$a} <=> $HRBlastresults->{$id1}->{$b} }  keys %{$HRBlastresults->{$id1}}){
               foreach my $rank2 ( sort {$HRBlastresults->{$id2}->{$a} <=> $HRBlastresults->{$id2}->{$b} }  keys %{$HRBlastresults->{$id2}}){
                  $HR->{$id1}->{$id2} = $HRBlastresults->{$id1}->{$rank1}->{subject} . "\t" . $HRBlastresults->{$id2}->{$rank2}->{subject};
                  $HR->{$id1}->{$id2} .= "\t" . $HRBlastresults->{$id1}->{$rank1}->{'%id'} . "\t" . $HRBlastresults->{$id2}->{$rank2}->{'%id'};
                  next LOOP;
               }
            }
         }
   ...
   foreach my $id1 (keys %{$HR}){
      foreach my $id2 (keys %{$HR->{$id1}}){
         print "$id1\t$id2\t$HR->{$id1}->{$id2}\n"; 
      } 
   } 
